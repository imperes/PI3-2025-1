...
esp_err_t ssd1306_clear_display(void)
{
    ...
    esp_err_t err = i2c_ssd1306_buffer_clear(&i2c_ssd1306);
    if (err != ESP_OK) {
        ESP_LOGE(SSD1306_TAG, "Failed to clear display buffer");
        return err;
    }
    
    // Update the physical display
    err = i2c_ssd1306_buffer_to_ram(&i2c_ssd1306);
    if (err != ESP_OK) {
        ESP_LOGE(SSD1306_TAG, "Failed to update display after clear");
        return err;
    }
